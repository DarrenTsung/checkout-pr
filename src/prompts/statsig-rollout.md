...
Use {{STATSIG_SKILL}} for live gate state and audit history. Treat `statsig-rollout.md` at the worktree root as the durable rollout dossier: read and preserve it if it exists, or create it if it does not. Preserve human-written text and previous journal entries. Refresh facts that have changed, but append a journal entry only when there is materially new evidence, a decision, an action, or a change in rollout state. Do not create duplicate entries on resume.
