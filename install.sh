...
# Symlink Claude commands (the flat files point at the shared SKILL.md sources)
CLAUDE_SKILLS_DIR="$HOME/.claude/commands/checkout"
...
for skill in "$SCRIPT_DIR"/skills/*.md; do
  ...
  ln -s "$skill" "$target"
  echo "Linked Claude command: checkout:$name"
done

# Symlink the same workflows as Codex skills.
